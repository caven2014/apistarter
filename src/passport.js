...
async function login(req, provider, profile, tokens) {
  ...
  console.log('login function ===> ', profile)
  ...
  if (!user) {
    user = await db
      ...
      .where({
        ...
        'logins.id': profile.id || profile.openid
      })
      .first('users.*');
    if (!user &&
      profile.emails &&
...
async function loginWechat(provider, profile, tokens) {
  ...
  if (!user) {
    ...
    if (!user &&
      ...
      user = await db
        .table('users')
        .innerJoin('wechat', 'wechat.open_id', 'wechat.nickname', 'users.id')
        .where({
          'wechat.open_id': profile.ipenid,
          'wechat.nickname': profile.nickname
        })
        .first('users.*');
    }
  }
  ...
  if (count === '0') {
    ...
    await db
      ...
      .update({
        username: profile.username,
        tokens: JSON.stringify(tokens),
        profile: JSON.stringify(profile._json) || null,
        updated_at: db.raw('CURRENT_TIMESTAMP'),
      });
  }
...
// https://www.npmjs.com/package/passport-wechat
// https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxc6e911cdded3962d&redirect_uri=http%3a%2f%2ff3934499.ngrok.io%2f&response_type=code&scope=snsapi_base&state=123#wechat_redirect
passport.use(new WechatStrategy({
    appID: 'wxc6e911cdded3962d',
    name: 'wechat',
    appSecret: '1ba02a7d9d1db6cbd10feb9936698b9d',
    client: 'wechat', // wechat web
    callbackURL: 'http://f3934499.ngrok.io/login/wechat/return',
  ...
  async (accessToken, refreshToken, profile, done) => {
    ...
    try {
      const user = await loginWechat(req, 'wechat', profile, {
        accessToken,
        refreshToken,
      });
      console.log(user)
